...
int nQueens(vector<vector<char>> &board , int row){
    ...
    for(int j = 0 ; j<n ; j++){
        if(isSafe(board , row , j)){
            ...
            board[row][j] = 'Q';
            count += nQueens(board , row+1); // place next queen in next row 
            board[row][j] = '.'; // remove the queen before trying next place  
        }
    }

    return count; // count oat each level accumaltes te no of solutions so level wise count and retuied and accumutlates to in the first crecucivse caklk parent call
}
